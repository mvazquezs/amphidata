# amphidata (development version)

## Funcions
* `elipsoide_metrics()`
- Funció per calcular les mètriques d'un elipsoide (area i perimetre segons diferents aproximacions).

# amphidata 0.0.1

## Carrega de dades originals
* Dades origininals presents en el directori `/inst/extdata/01_data_ori/`.

## Funcions
* `load_dimensions_vazquez()`: 
- Funció per carregar (usa dades internes), seleccionar, filtrar les dades i generar etiquetes.

* `load_dimensions_golvin()`: 
- Funció per carregar (usa dades internes), seleccionar, filtrar les dades i generar etiquetes.

# amphidata 0.0.0.9000

*To increase version use: 
- `usethis::use_version('dev')`
- `usethis::use_version('patch')`
- `usethis::use_version('minor')`
- `usethis::use_version('major')`

* To check the current version: `usethis::use_version()`

* To release a new version: `usethis::use_release_issue()`

* Initial CRAN submission.
